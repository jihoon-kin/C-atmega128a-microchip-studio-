...
void LCD_Data4bit(uint8_t data) // 상위비트와 하위비트를 둘다 enable 해줘서 데이터를 받아온것
// 8비트의 데이터를 보내주기 위한 함수 
{
...
//void LCD_Data4bitInit(uint8_t data) // 상위비트만 표시해주는것
//// 위에서 받은 8비트의 데이터를 enable 해주고 상위비트로만 뽑기위한 함수 // 이유는 우리가 4567번만 쓰기때문에 
//// 즉, 상위 4비트를 뽑아서 command 를 보내는 함수
//{
	//LCD_DATA_PORT = (LCD_DATA_PORT & 0x0f) | (data & 0xf0); // 상위 4비트를 LCD 데이터 핀에 출력
	//LCD_EnablePin(); // LCD 동작 신호 전송
//}
...
void LCD_Init() // HD44780 초기화 과정 -> data sheet 에 있음
{
	...
	LCD_WriteCommand(0x03); // 4비트 모드로 설정
